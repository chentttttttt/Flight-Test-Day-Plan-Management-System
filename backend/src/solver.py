...
def build_model(data):
    ...
    default_constraints = {
        ...
        "aircraft_cooling_hours": 1,    # 飞机最小间隔（小时），0 表示无冷却
        "pilot_cooling_hours": 1,       # 飞行员最小间隔（小时），0 表示无冷却
    ...
    aircraft_cooling = cfg.get("aircraft_cooling_hours", 1)
    pilot_cooling = cfg.get("pilot_cooling_hours", 1)
    ...
    for tid, info in task_info.items():
        dur = info['duration']
        risk = info['risk']
        aircraft_list = [info['aircraft']] if cfg["aircraft_binding"] else aircrafts
        ...
        for a in aircrafts:
            for s in valid_slots:
                prob += lpSum(
                    ...
                    for h in valid_slots
                    if (tid, a, h) in w and h <= s < h + task_info[tid]['duration'] + aircraft_cooling
                ) <= 1
    else:
        for a in aircrafts:
            for s in valid_slots:
        ...
        for p in pilots:
            for s in valid_slots:
                prob += lpSum(
                    ...
                    for a in aircrafts
                    for h in valid_slots
                    if (tid, p, a, h) in x and h <= s < h + task_info[tid]['duration'] + pilot_cooling
                ) <= 1
    else:
        for p in pilots:
            for s in valid_slots:
